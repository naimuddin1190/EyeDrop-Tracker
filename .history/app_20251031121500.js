/* app.js — বাংলা UI + ৪টি ড্রপের প্রগ্রেস বার + সম্পন্ন তালিকা PDF + warning message system */
...
function render(){
  ...
  day.forEach(it=>{
    ...
    tr.innerHTML=`<td>${it.time}</td>
                    ...
                    <td>${it.count}</td>
                    <td>${it.note}</td>
                    <td><button class="mark-btn ${it.taken?'done':'pending'}" data-idx="${it.idx}">${it.taken?'✅ সম্পন্ন':'⬜ অপেক্ষমাণ'}</button></td>`;
...
// Progress Bar + Warning/Success Message
function updateProgressUI(day){
  ...
  ['atrogen','vigalon','neopred','nevan'].forEach(tag=>{
    ...
    if(p===100 && !progBars[tag].dataset.completed){
      alert(`🎉 ${tag.toUpperCase()} drops completed today!`);
      progBars[tag].dataset.completed="true";
    }
    if(p<100) allComplete=false;
  });

  // সব ড্রপ সম্পূর্ণ হলে
...
function generateTablePDF() {
  const dateStr = viewDate.value;
  const day = loadDay(dateStr);
  let tableHtml = `<h2 style="margin:0 0 6px 0">চোখের ড্রপ রিপোর্ট</h2>
